...
1. El músico envía `proposal.v1` (1..K variantes) por ventana.
2. El researcher devuelve `critic_report.v1` (ranking + hints).
3. El orchestrator elige variante, aplica passes, y el PM valida con `constraints.v1`.
4. El resultado se integra al `score.v1`.

### Schemas disponibles

- `schemas/proposal.schema.v1.json` - Propuestas del Musician
- `schemas/critic_report.schema.v1.json` - Reportes del Researcher  
- `schemas/constraints.schema.v1.json` - Constraints del PM
- `schemas/score.schema.v1.json` - Score final
